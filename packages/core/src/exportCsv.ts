 ...
 *
 * 注意：CSV 词表只能承载 term / definitions / pos 三列；音标、标签、例句、
 * 记忆状态等字段不在 CSV 范围内（完整备份请用 exportLexilexiData 的 JSON）。
...
/** 导出当前词表为 CSV 文本（仅未删除条目，按 createdAt 升序） */
export async function exportCsvWordlist(db: LexilexiDatabase): Promise<string> {
  ...
  return serializeWordlistCsv(entries);
}
