...
export interface LexiiExportData {
  format: "lexii";
...
export async function importLexiiData(
  ...
  if (data.dbSchemaVersion > DB_SCHEMA_VERSION) {
    throw new Error(
      `导出数据由更新版本的 Lexii 生成（schema v${data.dbSchemaVersion} > 当前 v${DB_SCHEMA_VERSION}），请先升级应用`,
    );
  }
 ...
 *
 * 只做结构校验（未知键保留），schema 版本校验由 importLexiiData 负责；
