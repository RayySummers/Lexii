 ...
 *   - 「专四冲刺」= tag ∈ {cet6,toefl} 合并去重（基准合并实测 9,137 词）；
 *   - 「专八冲刺」= tag ∈ {gre,toefl,ielts} 合并去重（基准合并实测
 *     12,842 词），按词频（frq desc → bnc desc → term asc 确定性并列
 *     裁决）截断至与「专四冲刺」同词数（与专四冲刺同量级）；
 *   - RAY-274 调整：GRE 剔除 + P1 真题高频词补入 + P1.5 基础通用词补入
 *     （口径与实现见 lib/books.mjs），两本冲刺词书实测均 8,733 词；
