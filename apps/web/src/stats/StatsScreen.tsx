/**
 * 统计页：10 项统计（RAY-252：统计面板扩充 + RAY-270：学习时长 2 项）+
 * 导航改版（RAY-253）。
 *
 * 连续天数 / 累计天数 / 今日已学习（次数）/ 今日已复习（次数）/
 * 今日学习时长 / 今日待学（词条）/ 明日到期（词条）/ 累计已完成（次数）/
 * 累计已完成（词条）/ 累计学习时长。
 *
 ...
 * 口径，另有额度提示说明二者关系）。
 *
 * RAY-270：新增今日/累计学习时长两项。时长由 review 事件的有效时长聚合
 * （口径见 packages/stats：单卡超上限截断、闲置不计），经
 * formatStudyDuration 自适应显示——不足 1 小时「X 分钟」、1 小时及以上
 * 「X 小时 Y 分钟」。撤销评分删除事件，时长随事件流重算自然一致。
 *
...
/** 10 项统计的展示顺序与文案（口径与 packages/stats README 对齐） */
const STAT_ROWS: ReadonlyArray<{ label: string; value: (stats: StatsSnapshot) => string }> = [
  { label: "连续天数", value: (stats) => String(stats.streakDays) },
  { label: "累计天数", value: (stats) => String(stats.totalDays) },
  { label: "今日已学习（次数）", value: (stats) => String(stats.todayLearnCount) },
  { label: "今日已复习（次数）", value: (stats) => String(stats.todayReviewCount) },
  { label: "今日学习时长", value: (stats) => formatStudyDuration(stats.todayStudyDurationMs) },
  { label: "今日待学（词条）", value: (stats) => String(stats.newCardsRemainingToday) },
  { label: "明日到期（词条）", value: (stats) => String(stats.dueTomorrowCount) },
  { label: "累计已完成（次数）", value: (stats) => String(stats.reviewCount) },
  { label: "累计已完成（词条）", value: (stats) => String(stats.completedWordCount) },
  { label: "累计学习时长", value: (stats) => formatStudyDuration(stats.totalStudyDurationMs) },
];
    ...
    <main className="mx-auto flex w-full max-w-2xl flex-col gap-6 px-4 py-8 sm:px-6">
      ...
      {error ? (
        ...
        <section aria-label="学习统计" className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {STAT_ROWS.map((row) => (
            <StatCard key={row.label} label={row.label} value={row.value(stats)} />
          ))}
        </section>
      )}
    </main>
