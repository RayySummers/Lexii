    ...
    <main className="mx-auto flex w-full max-w-2xl flex-col gap-6 px-4 py-8 sm:px-6">
      ...
      <div className="relative">
        ...
        <input
          ...
          autoCapitalize="none"
          spellCheck={false}
          className="w-full rounded-full border border-outline-variant bg-surface py-3 pl-11 pr-4 text-base text-on-surface placeholder:text-on-surface-variant transition-colors hover:border-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
        />
      </div>

      {queryStack.length > 0 ? (
        <div className="flex items-center gap-2 rounded-md border border-outline-variant bg-surface-container px-4 py-2 text-xs text-on-surface-variant">
          <span className="truncate">来自「{queryStack[queryStack.length - 1]}」的近义词跳转</span>
          <button
            type="button"
            onClick={handleBack}
            className="ml-auto shrink-0 rounded-full border border-outline-variant bg-surface px-3 py-1 text-xs font-medium transition-colors hover:border-primary hover:text-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
          >
            返回
          </button>
        </div>
...
function SearchContent({
  ...
  if (searching && results === null) {
    return (
      <div role="status" className="py-12 text-center text-sm text-on-surface-variant">
        正在检索…
      </div>
    );
  }
  if (results !== null && results.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-lg border border-outline-variant bg-surface p-8 text-center">
        ...
        {onNavigateToSettings ? (
          <button
            ...
            className="mt-1 rounded-full border border-primary/40 bg-primary/10 px-4 py-2 text-sm font-medium text-primary transition-colors hover:bg-primary/20 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
          >
            前往设置安装扩展词包
          </button>
        ) : null}
...
function SearchHistoryList({ history, onSelect, onRemove }: SearchHistoryListProps) {
  return (
    <section aria-label="搜索历史" className="flex flex-col gap-2">
      <h2 className="px-1 text-sm font-medium text-on-surface-variant">搜索历史</h2>
      <ul className="flex flex-col gap-2">
        {history.map((term) => (
          <li
            key={term}
            className="flex items-center gap-1 rounded-md border border-outline-variant bg-surface py-1.5 pl-4 pr-1.5"
          >
            <button
              type="button"
              onClick={() => onSelect(term)}
              title={term}
              className="min-w-0 flex-1 truncate py-1 text-left text-base text-on-surface transition-colors hover:text-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
            >
              {term}
            </button>
            <button
              ...
              className="shrink-0 rounded-full p-2 text-on-surface-variant transition-colors hover:bg-surface-container hover:text-on-surface focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
            >
...
function SearchResultRow({
  ...
  return (
    <li className="flex flex-col gap-1.5 rounded-md border border-outline-variant bg-surface p-4">
      <span className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
        ...
        {result.source === "dictionary" ? (
          <span className="rounded-full border border-tertiary/30 bg-tertiary/10 px-2 py-0.5 text-xs text-tertiary">
            扩展词典
          </span>
        ) : null}
      </span>
      <p className="text-sm leading-relaxed text-on-surface-variant">{sense.definitions.join("；")}</p>
      {synonymGroups.length > 0 ? (
        <div className="flex flex-col gap-2 rounded-sm border border-outline-variant/60 bg-surface-container/40 px-3 py-2">
          <span className="text-xs font-medium text-on-surface-variant">近义词</span>
          <div className="flex flex-col gap-2">
            {synonymGroups.map((group) => (
              <div key={group.definitionIndex} className="flex flex-col gap-1">
                <span className="flex items-center gap-1 text-xs text-on-surface-variant">
                  {group.pos ? (
                    <span className="rounded-full border border-outline-variant bg-surface px-1.5 py-px text-xs">
                      {group.pos}
                    </span>
                  ) : null}
                      ...
                      <li key={`${index}:${word}`}>
                        {clickable ? (
                          <button
                            ...
                            title={`搜索「${word}」`}
                            className="rounded-full border border-outline-variant bg-surface px-2.5 py-0.5 text-xs transition-colors hover:border-primary hover:text-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
                          >
                            {word}
                          </button>
                        ) : (
      ...
      <div className="flex items-center justify-end gap-2">
        <button
          ...
          className="flex items-center gap-1 rounded-full border border-outline-variant bg-surface px-4 py-1.5 text-xs font-medium text-on-surface-variant transition-colors hover:border-primary hover:text-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
        >
        ...
        <button
          ...
          className={`flex items-center gap-1 rounded-full border px-4 py-1.5 text-xs font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary ${
            inNotebook
