 ...
 *
 * 颜色值不在此处硬编码：统一读取 tokens.css 中的语义 token --lex-background
 * （浅色/深色两套，随 <html data-theme="dark"> 切换自动生效），
 * meta 内容因此始终与页面背景 token 一致，不存在第二份颜色定义。
 *
 * - index.html 保留浅色初始值（--lex-background 浅色值）用于首帧渲染前与无 JS 兜底，
 *   深色值只在运行时从 token 读取——任何 JS/HTML 都不会新增硬编码颜色。
 * - themeColor.test.ts 校验：本模块源码不含十六进制颜色、index.html 初始值与
 *   tokens.css 的浅色 --lex-background 一致，防止两处漂移。
 */
export const THEME_COLOR_META_SELECTOR = 'meta[name="theme-color"]';

/** 读取当前主题下 --lex-background token 的值（CSS 尚未加载或环境不支持时为 ""） */
export function readThemeColorToken(doc: Document = document): string {
  const view = doc.defaultView;
  if (!view) {
    return "";
  }
  return view.getComputedStyle(doc.documentElement).getPropertyValue("--lex-background").trim();
}

/**
 * 将 meta theme-color 同步为当前主题的 --lex-background token 值。
