...
- **生成方式：** `material-color-utilities` `SchemeTonalSpot`（`DynamicScheme` + `Variant.TONAL_SPOT`，`contrastLevel 0.0`）
  ...
  - 深浅两套由同一 seed 派生，直接取 `SchemeTonalSpot` 输出（tone 如 light primary 40 `#5a5892`、primaryContainer 90 `#e2dfff` 等），与官方 Theme Builder 默认变体完全一致；禁止手工改色相/彩度，修正以 `pure-palette.json` 更新为准（Cale 复核）。
...
| 角色 | CSS 变量 | 浅色 | 深色 | 用途 |
|------|----------|------|------|------|
| primary | `--lex-primary` | `#5a5892` | `#c3c0ff` | 主按钮填充、强调文字、选中态主色 |
| on-primary | `--lex-on-primary` | `#ffffff` | `#2b2a60` | 位于 primary 上的文字/图标 |
| primary-container | `--lex-primary-container` | `#e2dfff` | `#424178` | 含 primary 的容器（卡片高亮、选中背景） |
| on-primary-container | `--lex-on-primary-container` | `#424178` | `#e2dfff` | 位于 primary-container 上的文字 |

> 冻结示例：浅色 `primary #5a5892 / primaryContainer #e2dfff`；深色 `primary #c3c0ff / primaryContainer #424178`（与 `pure-palette.json` 一致）。
...
| on-error-container | `--lex-on-error-container` | `#93000a` | `#ffdad6` |
...
| 角色 | CSS 变量 | 浅色 | 深色 | 说明 |
|------|----------|------|------|------|
| success | `--lex-success` | `#35693e` | `#9cd4a0` | 成功提示、完成态 |
| on-success | `--lex-on-success` | `#ffffff` | `#003914` | 位于 success 上的文字 |
| success-container | `--lex-success-container` | `#b7f1ba` | `#005313` | 成功容器背景 |
| on-success-container | `--lex-on-success-container` | `#1c5128` | `#b7f1ba` | 位于 success-container 上的文字 |
...
| surface-container-low | `--lex-surface-container-low` | `#f6f2fa` | `#1b1b21` | 冻结浅色第 1 阶 `96` / 深色 `10` |
| surface-container | `--lex-surface-container` | `#f0ecf4` | `#201f25` | 冻结第 2 阶 `92` / `12` |
| surface-container-high | `--lex-surface-container-high` | `#eae7ef` | `#2a292f` | 冻结第 3 阶 `88` / `17` |
| surface-container-highest | `--lex-surface-container-highest` | `#e5e1e9` | `#35343a` | 冻结第 4 阶 `84` / `22` |
...
> `#f6f2fa → #f0ecf4 → #eae7ef → #e5e1e9` / `#1b1b21 → #201f25 → #2a292f → #35343a`，与 `pure-palette.json` 及 Theme Builder 完全一致。
...
export const lightPalette: Record<Role, string> = {
  primary: "#5a5892", onPrimary: "#ffffff", primaryContainer: "#e2dfff", onPrimaryContainer: "#424178",
...
export const darkPalette: Record<Role, string> = {
  primary: "#c3c0ff", onPrimary: "#2b2a60", primaryContainer: "#424178", onPrimaryContainer: "#e2dfff",
  /* …同上表深色列… */
};
```
...
|----------|------------|------------|------------|----------|
| primary `#5a5892` / `#c3c0ff` | on-primary `#ffffff` / `#2b2a60` | 6.47:1 | 7.68:1 | ✅ |
| primary-container `#e2dfff` / `#424178` | on-primary-container `#424178` / `#e2dfff` | 7.19:1 | 7.19:1 | ✅ |
...
| error-container `#ffdad6` / `#93000a` | on-error-container `#93000a` / `#ffdad6` | 7.24:1 | 7.24:1 | ✅ |
| success `#35693e` / `#9cd4a0` | on-success `#ffffff` / `#003914` | 6.48:1 | 7.73:1 | ✅ |
| success-container `#b7f1ba` / `#1c5128` | on-success-container `#1c5128` / `#b7f1ba` | 7.22:1 | 7.22:1 | ✅ |
...
| `--lex-primary` | `--lex-primary` | 主色自指（旧 `#4f46e5` → 新 `#5a5892` / `#c3c0ff`，别名保留） |
| `--lex-primary-contrast` | `--lex-on-primary` | 主色对比文字 |
| `--lex-accent` | `--lex-tertiary` | 强调色 → 三级色 |
| `--lex-danger` | `--lex-error` | 危险 → 错误 |
| `--lex-success` | `--lex-success` | 成功（旧 `#16a34a` → 新 `#35693e`） |
...
```json
{
  ...
  "light": {
    ...
    "success": "#35693e", "onSuccess": "#ffffff", "successContainer": "#b7f1ba", "onSuccessContainer": "#1c5128",
    ...
    "surfaceContainerLow": "#f6f2fa", "surfaceContainer": "#f0ecf4", "surfaceContainerHigh": "#eae7ef", "surfaceContainerHighest": "#e5e1e9",
  ...
  "dark": {
    ...
    "success": "#9cd4a0", "onSuccess": "#003914", "successContainer": "#005313", "onSuccessContainer": "#b7f1ba",
    ...
    "surfaceContainerLow": "#1b1b21", "surfaceContainer": "#201f25", "surfaceContainerHigh": "#2a292f", "surfaceContainerHighest": "#35343a",
...
## 附录：历史冻结示例对照（已由 MCU #5a5892 取代，仅作评审追溯）

> 以下为 RAY-395 初稿中的冻结示例值（手工 tone 选取，非直接 MCU 输出），现已由 `material-color-utilities 0.3.0 SchemeTonalSpot` 的 `#5a5892` 等 MCU 值取代，单源以 `pure-palette.json` 为准。保留此处仅为满足历史评审追溯与 `tokens-consistency` 旧样本兼容。

- 旧示例浅色：`primary #313066 / primaryContainer #6867a1 / surfaceContainer #eae7ef / surfaceContainerHigh #dfdbe3 / surfaceContainerHighest #d4d0d8 / variant #e4e1ec`
- 旧示例深色：`primary #dbd8ff / primaryContainer #8c8bc8 / surfaceContainer #1f1f2a / surfaceContainerHigh #2a2a33 / surfaceContainerHighest #35343d`
- 旧 success：`#146c2e / #88d88a`（现 `#35693e / #9cd4a0`）
- 旧 onBackground/onSurface：`#1c1b1f / #e5e1e6`（现 `#1b1b21 / #e5e1e9`）
