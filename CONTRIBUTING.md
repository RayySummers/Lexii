...
PR 描述使用下方模板，并自行对照「13 条评审标准」在描述中逐项勾选自查结果。CI（build + test + typecheck + lint + fsrs-verify）必须全绿；`main` 分支受保护，禁止直推。评审通过为合入惯例，非 GitHub 强制门槛（原因见「评审流程」）。
...
5. **合并要求**：CI 全绿为强制门槛，评审通过为合入惯例；合并前禁止直推 `main`（分支保护）。仓库未开启「评审通过数 ≥ 1」的分支保护：Multica 协作模式下全体 Agent 与所有者共用同一 GitHub 账户，开启后任何人都无法批准自己的 PR——故评审以流程惯例执行，不靠 GitHub 强制。
