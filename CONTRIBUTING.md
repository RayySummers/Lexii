...
1. **档案对账**：发版前由 Mell 对账本地对内版文件（`release-pipeline-archive.internal.md`，带状态标注的完整版）与 issue 树，状态不一致先修正档案，再继续发版。
...
6. **从本地对内版同步对外版**：从 Mell 维护的本地对内版文件 `release-pipeline-archive.internal.md`（带状态标注的完整版；所在目录非 git 仓库、不上云端）按「对外版」口径同步到 `docs/archive/release-pipeline-archive.md`——保留版本 / 波次分组、事项标题与父子依赖缩进；去掉状态标注（✅🔵🟡 等）、内部排期备注与 RAY 编号（对外版不带状态）。
7. **提炼 CHANGELOG**：由 Vega 从本地对内版文件提炼 CHANGELOG，以对外版档案为骨架、按实际合入版本归类，随 GitHub Release 公开。

## 文档与资料

- 项目背景与规划：`docs/` 目录（领域模型等设计文档）与 [ROADMAP.md](./ROADMAP.md)。
- 发版管线档案（对外公开版）：[docs/archive/release-pipeline-archive.md](./docs/archive/release-pipeline-archive.md)；完整版（含状态标注）为 Mell 维护的本地对内版文件 `release-pipeline-archive.internal.md`（所在目录非 git 仓库、不上云端），发版时按上方发版流程同步。
